...
SeSwapChain::SeSwapChain(SeDevice &device, VkExtent2D extent) : device{device}, windowExtent{extent} {
  init();
}

SeSwapChain::SeSwapChain(SeDevice &device, VkExtent2D extent, std::shared_ptr<SeSwapChain> previous) : device{device}, windowExtent{extent}, oldSwapChain{previous} {
    init();
...
void SeSwapChain::createRenderPass() {
    // Render pass is like a blueprint that tells a graphics pipeline object what layout to expect for a frame buffer
